...
class ItemModel extends Equatable {
  ...
  factory ItemModel(ItemSpec itemSpec) {
    ...
    final fieldModels = UnmodifiableListView(itemSpec.fields.map((e) => e.accept(fieldModelCreatorVisitor)).toList(growable: false));
  ...
  String getOverviewListTitle() {
    final titleFieldModel = fields.firstWhere((element) => element.spec == itemSpec.overviewListTitleField);
    final titleFieldDefaultModel = defaults.firstWhere((element) => element.spec == itemSpec.overviewListTitleField);
    if (titleFieldDefaultModel != titleFieldModel) {
      return titleFieldModel.value.toString(); // FIXME title string representation for all field types
    }
  ...
  String? getOverviewListSubtitle() {
    ...
    final subtitleFieldModel = fields.firstWhere((element) => element.spec == itemSpec.overviewListSubtitleField);
    final subtitleFieldDefaultModel = defaults.firstWhere((element) => element.spec == itemSpec.overviewListSubtitleField);
    if (subtitleFieldDefaultModel != subtitleFieldModel) {
      return subtitleFieldModel.value.toString(); // FIXME title string representation for all field types
    }
    return null;
  }
  ...
  ItemModel copy(FieldModel newFieldModel) {
    final newFields = UnmodifiableListView(fields.map((e) => e.spec == newFieldModel.spec ? newFieldModel : e).toList());
    return new ItemModel._(this.itemKey, this.itemSpec, newFields, this.defaults);
  }
}
